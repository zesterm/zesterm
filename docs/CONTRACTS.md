...
| `ClientMessage`, `HostMessage`, `SessionInfo`, `HostOffer`, `HostProfile`, `ConfigValue`, `ConfigField`, `ConfigVariant`, `ConfigProfile`, `ConfigTheme`, `ConfigOp` | `crates/zest-proto/src/lib.rs` | **frozen** at v3, additively extended (`watch_pairings`, `watch_hosts`, `watch_signals`, `Sessions.offer`, `Attach.observe`, `HostMessage::Attention`, `HostMessage::Progress`, the loopback-scoped `Enroll`/`EnrollResult` pair, the cwd browser's `ListDir`/`DirListing` pair (#439), `HostOffer.has_account_token`, `SessionInfo.context`/`busy` with the `SessionContext` family beside it, #416; the editor's `ReadFile`/`FileContents` and `WriteFile`/`FileWritten` pairs, #446, and `GitDiff`/`GitDiffResult` beside them, #453; `CreateSession.env`, #488 — skipped when empty, so an ordinary launch is byte-identical to what a peer predating it sent and the conformance fixtures do not move; `Hello.agent`, a client declining device authority for itself, #491; the config surface's `GetConfig`/`ConfigState` and `SetConfig`/`ConfigWritten` pairs, #509 — a value crosses as its **TOML spelling in a `String`**, because `zest-proto` must not gain `serde_json` and because the write direction hands the same text back to `toml_edit`, so `14` stays `14` in a hand-written file; `ConfigProfile` carries `host`/`ask_host` where `HostProfile` deliberately drops them, one being a *view of a file* and the other a *launch target* the viewer resolves against its own fleet — ADR-014, and the one place these two types must not be merged; `CreateSession.env` and `CreateSession.profile`, #488 and #496 — both skipped when empty, so an ordinary launch is byte-identical to what a peer predating them sent and the conformance fixtures do not move — and `profile`'s meaning widened in #559 without the shape moving: a host that has a profile by that name now applies its own `env` *beneath* the launch's entries, which is what lets a client launch a profile it learned from the offer, since `HostProfile` publishes none (a name the host lacks still changes nothing beyond expansion; `clients/web` does not yet send it, #560); `ConfigProfile.env`, #559 — the merged, unexpanded map a launch uses, the read half of what `SetConfig` could already write; the fleet search's `SearchBlocks`/`BlockMatches` pair, #527 — the daemon answers for its own live sessions and a client asks every host it holds a connection to, so "the fleet" is the client's merge and never a daemon's claim). **A new `HostMessage` tag is only additive behind a `Hello` flag** — an older peer cannot decode the frame and `DaemonClient::recv` maps that to a transport error, which ends the connection; `watch_signals` is what keeps `Attention` from reaching one. **The exception is a tag that is only ever a *reply*** — `EnrollResult`, `DirListing`, `FileContents`, `FileWritten`, `GitDiffResult`, `ConfigState`, `ConfigWritten`, `BlockMatches` — which reaches a peer only in answer to a request that peer had to send first, so a peer too old to decode it is structurally unable to ask for it; those need no flag, and the matching `ClientMessage` tag degrades through the daemon's generic could-not-understand `Error` | `zest-daemon`, `zest-app`, `zest-mcp`, `clients/web` |
...
| `Block`, `BlockIndex`, `BlockState` | `crates/zest-core/src/blocks.rs` | **frozen**, additively extended: `Block.context` (`BlockContext`, embedder-stamped, #429), `Block.author` (32 opaque bytes, embedder-stamped, #491 — **not** a `ClientId`: `zest-proto` depends on this crate, so the wire converts, as `LineId` becomes `i64` there). `Block::is_command` and `BlockIndex::last_command` (#533) are the *rule* rather than the shape: which block ran something, and which one therefore names a session. It has a second implementation, `GridView.lastCommand` in `clients/web/packages/proto/src/grid-view.ts`, and the two change together — the `Predictor` row's discipline, for the same reason: a client that answered it differently would name a tab something the host never ran | `zest-app`, `zest-daemon`, `zest-mcp`, `clients/web` |
...
| `BlockMatch` | `crates/zest-proto/src/search.rs` | **frozen** at creation (#527), additively extended. A search hit, deliberately *not* `BlockPayload`: no line ids (they mean nothing off the grid that issued them, and a stored block of a dead session has none), `session` is `Option` so a block only a store remembers can never be activated, and every option is a plain `null` rather than a skipped key because the message is a reply and never rides a delta. `search::Needle`/`rank` beside it are the one match-and-order rule the daemon, the app, `zest-mcp` and the store all apply; additively extended with `command_truncated` (`#[serde(default)]`, #529 — a stored command the host cut, which a client must not re-run as if whole) | `zest-daemon`, `zest-app`, `zest-mcp`, `clients/web` |
