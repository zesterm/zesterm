...
fn main() {
    ...
    if let Some(code) = opt("--enroll") {
        ...
        match enroll::enroll(&identity, &code, &label, &base, &http, store.as_ref()) {
            ...
            // `refusal_text` turns wrong_kind (#228) and already_enrolled's
            // two causes (#368) into the person's next move; everything else
            // it renders as Display, so this arm is every failure.
            Err(e) => {
                eprintln!("zest-daemon: {}", enroll::refusal_text(&e));
                std::process::exit(1);
            }
        }
        return;
...
fn machine_label_from(env: impl Fn(&str) -> Option<String>) -> String {
    ...
    if let Some(name) = zest_daemon::context::os_hostname() {
