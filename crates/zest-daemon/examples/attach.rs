    ...
    loop {
        ...
        while let Some(body) = reader.next_frame().expect("framing") {
            ...
            match frame::decode::<HostMessage>(&body).expect("decode") {
                ...
                // This probe drives a session and asks no question of its
                // own (#439, #446, #453), so any of these would be a daemon
                // talking to itself. Named rather than swept into a `_` arm,
                // so the next tag is still a compile error here.
