...
triple-click a line, Alt-drag for a rectangle. Ctrl+Shift+C copies and
Ctrl+Shift+V pastes — and on Windows plain **Ctrl+V** pastes too, as it does in
every other terminal there, which leaves Alt+V to whatever is running inside.
Right-click copies when there is a selection and pastes otherwise; middle-click
pastes. When the clipboard holds a *picture* rather than text, the same paste
(⌘V on macOS) writes it to a file and pastes the path — which is what dropping
the image on the window would have delivered, so a shell, an editor and an agent
all get something they can open. Middle-click is unaffected: it pastes the
selection, which is always text.
