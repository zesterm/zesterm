...
| `BlockMatch` | `crates/zest-proto/src/search.rs` | **frozen** at creation (#527), additively extended. A search hit, deliberately *not* `BlockPayload`: no line ids (they mean nothing off the grid that issued them, and a stored block of a dead session has none), `session` is `Option` so a block only a store remembers can never be activated, and every option is a plain `null` rather than a skipped key because the message is a reply and never rides a delta. `search::command_matches`/`rank` beside it are the one match-and-order rule the daemon, the app, `zest-mcp` and the store all apply | `zest-daemon`, `zest-app`, `zest-mcp`, `clients/web` |
