...
| `FleetHost`, `SessionsState`, `HostRoute`, `best_route` | `crates/zest-fleet/src/lib.rs` | draft — one consumer today (`zest-app`), and it freezes when `zest-mcp` becomes the second. The constraint to preserve meanwhile: every field stays `pub` and the struct keeps no private state, because a consumer outside `zest-app` builds these rows itself from mDNS and the account listing rather than receiving them from a `FleetModel` | `zest-app` |
